...
export const MacbookScroll = ({
  ...
  const { scrollYProgress: scrollYProgressNoOffset } = useScroll({
    target: ref,
  })
  ...
  const textOpacity = useTransform(scrollYProgressNoOffset, [0, 0.3], [1, 0])

  return (
    <div
      ref={ref}
      className="relative min-h-[180vh] flex flex-col items-center pt-32 justify-start flex-shrink-0 [perspective:800px] transform md:scale-100 scale-[0.35] sm:scale-50"
    >
      <motion.h2
        style={{
      ...
      />
      {/* Base area */}
      <div className="h-[22rem] w-[32rem] bg-slate-700 rounded-2xl overflow-hidden relative -z-10">
        {/* above keyboard bar */}
        ...
        <Trackpad />
        <div className="h-2 w-20 mx-auto inset-x-0 absolute bottom-0 bg-gradient-to-t from-slate-700 to-slate-900 rounded-tr-3xl rounded-tl-3xl" />
        {showGradient && (
          <div className="h-40 w-full absolute bottom-0 inset-x-0 bg-gradient-to-t from-slate-900  via-slate-700 to-transparent z-50"></div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="relative [perspective:800px]">
      ...
      >
        ...
        <Image
          src={src as string}
          alt="Cobalto app demo"
          fill
          className="object-cover object-left-top rounded-lg top-20 inset-0 h-full w-full"
        />
      </motion.div>
    </div>
  )
